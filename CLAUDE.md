...
- **环境隔离 + 泳道覆盖（2026-08-30，spec `docs/superpowers/specs/2026-08-30-configcenter-env-lane-design.md`）**：补齐环境维度（同应用 test/prod 各一份独立配置/版本/闸门）与泳道 key 级覆盖（merge 发现）。merge 链只两层：`app×env（版本化基线）→ lane（key 级覆盖）`，shared 不参与；lane 覆盖无版本链（即时生效、随泳道回收消失，物理删）。**数据模型**：`Namespace.EnvID`（migration 0038，`cc_namespaces.env_id` + 唯一索引 `(tenant,app,env) WHERE app_id!=''`，存量 app ns env_id='' 语义=「默认配置」）+ 新实体 `LaneOverride`（`cc_lane_overrides` 表，唯一五元组）；app scope 懒建 `EnsureByAppEnv(ctx, appID, envID)`（ns 名 `app-<appID>-<envID>`，envID 空保持 `app-<appID>` 兼容）。**发现协议**：`GET /api/configcenter/apps/{name}/published?env=&lane=`——env 精确→'' 回退→{}；服务端 `MergeSnapshot`（merge.go 纯函数）+ `overrideHash`（FNV-1a 排序 key=value 串，无覆盖省略字段）；version+overrideHash 双指纹，客户端任一变化即热替换（纯 lane 覆盖变化不改 version）。不带 env/lane 行为与升级前完全一致。**REST**：全部应用维度端点加 `?envId=`（写路径 `EnsureByAppEnv` 懒建，读路径 env→'' 回退）；新增 lane-overrides 三端点（GET/POST upsert/DELETE `/{key}`，lane 必填）；生产闸门 `allowProd`（AppHandler 注入 `WithEnvResolver`，env type=prod 写操作需 `prod:write`，解析失败 fail-closed 按生产；lane 覆盖写同款）。**泳道回收级联**：`LaneOverrideCleaner` 接口（configcenter 定义，`CleanLane(ctx, tenantID, envID, laneID)` 按 (env,lane) 跨 app 清）+ `LaneGC.Cleaners` 字段（workload 侧同形接口不 import configcenter），Sweep 与 ReclaimLane 双路径在「泳道全组删完」时机级联清（与 MarkClosed 同时机，best-effort 失败仅日志；部分删除/MaxSweep 截断不清）；cmd/core gc 构造单点注入。**前端**：AppDynamicConfigs 顶栏 env 贯穿 + 泳道覆盖子区（即时生效标注）+ isProd confirmDangerous。**dogfooding**：chatbot dynconfig 取 `PAAS_CONFIG_ENV`（controller service 类型 Pod 注入，与 DPToken 同块）+ `PAAS_LANE_ID`（已有约定）带 env/lane query 发现 + version/overrideHash 双比对热替换，泳道部署实例自动拉到覆盖后配置。留后续：存量 env='' draft 项迁移、EnsureByAppEnv 悬挂 app 校验、shared 引用层。
