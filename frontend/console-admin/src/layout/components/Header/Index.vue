...
const handleDropdownCommand = async (command: string) => {
  if (command === 'profile') {
    ...
    const confirmed = await confirmService.showConfirm('确定要退出登录吗？')
    if (!confirmed) return
    // 清理认证、用户、菜单状态
    // logout 接口失败不阻塞：auth 请求 _silent 无全局提示，后端失败时仍静默清本地态并跳登录页
    try {
      await authService.logout()
    } catch (e) {
      console.debug('[logout] backend failed, clearing locally', e)
    } finally {
      userStore.reset()
