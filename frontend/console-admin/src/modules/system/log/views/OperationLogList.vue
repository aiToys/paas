  ...
  >
    <template #search>
      <el-input
        ...
        :placeholder="t('log.operationKeyword')"
        clearable
        style="width: 200px"
...
const handleClear = async () => {
  const confirmed = await confirmService.showConfirm(t('log.confirmClearOperation'))
  if (!confirmed) return
  try {
...
const handleExport = async () => {
  try {
    ...
    downloadCsv(csv, t('log.operationFileName'))
...
const columns = computed<ColumnDef[]>(() => [
  ...
  { prop: 'module', label: t('permission.field.module'), minWidth: 120 },
  { prop: 'operation', label: t('log.field.operation'), minWidth: 120 },
  { prop: 'method', label: t('log.field.requestMethod'), minWidth: 100 },
  { prop: 'ip', label: t('log.field.ip'), minWidth: 140 },
  { prop: 'location', label: t('log.field.operationLocation'), minWidth: 150 },
  { prop: 'time', label: t('log.field.duration'), minWidth: 90, slot: 'time' },
  { prop: 'status', label: t('common.column.status'), minWidth: 90, slot: 'status' },
  { prop: 'operationTime', label: t('log.field.operationTime'), minWidth: 180, slot: 'operationTime' },
])
