...
// 对接 core /api/tenants（平台级，super_admin 可访问；普通 tenant-admin 受限本租户）。
...
// 获取租户列表（对接 core GET /api/tenants；假分页 + keyword 过滤，适配 useCrud）。
export const fetchTenantList = async (
  params: TenantSearchRequest
): Promise<TenantSearchResponse> => {
  const list = await api.get<CoreTenant[]>('/api/tenants')
...
// 创建租户（对接 core POST /api/tenants；id+name 必填，id 全局唯一）。
export const createTenant = async (data: TenantCreateRequest): Promise<TenantInfo> => {
  const t = await api.post<CoreTenant>('/api/tenants', data)
  return mapTenant(t)
}

// 删除租户（对接 core DELETE /api/tenants/{id}；租户下有用户时 core 返 409 引导先清用户）。
