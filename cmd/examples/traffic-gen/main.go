...
//
// 环境变量：
//   CORE_URL        core API 地址（如 http://paas-core.paas.svc:8080）
//   API_KEY         平台 API Key（程序化调用，绑 developer 角色）
//   AGENT_MODEL     Agent 虚拟模型（如 agent:xxx）
//   REC_SVC_URL     推荐服务 URL（K8s Service，如 http://rec-svc.paas.svc:80）
...
func runLoops() {
	...
	coreURL := env("CORE_URL", "http://paas-core.paas.svc:8080")
	apiKey := env("API_KEY", "")
	agentModel := env("AGENT_MODEL", "")
	microInterval := envInt("MICRO_INTERVAL", 300)
	agentInterval := envInt("AGENT_INTERVAL", 600)

	log.Printf("[traffic-gen] 启动常驻模式：micro=%ds agent=%ds rec=%q agent=%q", microInterval, agentInterval, recSvcURL, agentModel)

	if recSvcURL != "" {
		go loop("micro", microInterval, func() { callMicro(recSvcURL) })
	} else {
		log.Println("[traffic-gen] REC_SVC_URL 未设置，跳过微服务流量")
	}
...
func callMicroOnce() {
	recSvcURL := env("REC_SVC_URL", "")
	if recSvcURL == "" {
		log.Println("[traffic-gen:once] REC_SVC_URL 未设置，退出")
		return
	}
	callMicro(recSvcURL)
}

// callMicro 调推荐服务链路（rec-svc），保障微服务调用链有流量。
func callMicro(recSvcURL string) {
	client := &http.Client{Timeout: 10 * time.Second}
	resp, err := client.Get(recSvcURL)
	if err != nil {
		log.Printf("[micro] rec-svc 调用失败: %v", err)
		return
	}
	defer resp.Body.Close()
	_, _ = io.Copy(io.Discard, resp.Body)
	log.Printf("[micro] rec-svc %s -> %d", recSvcURL, resp.StatusCode)
}
