...
func serveHTTP(gw *gateway.Gateway, meter *gateway.Meter, stores *Stores, appliers k8sAppliers, metricsReg *metrics.Registry, laneReclaimer *workload.LaneGC) *http.Server {
	...
	reg.Operation("POST", "/api/applications/{id}/dynamic-configs/shared-refs", apiroute.Tags("配置中心"), apiroute.Summary("引用共享配置（body {sharedNsId}，?envId=，prod 需 prod:write；发现时 shared 作为三层 merge 基础层）"), apiroute.Perm("application:write"), apiroute.WithReqBody(configcenter.NSRef{}), apiroute.WithResp(configcenter.NSRef{}))
